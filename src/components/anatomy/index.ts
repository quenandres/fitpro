export { AnatomyViewer, default } from './AnatomyViewer';
export type { AnatomyViewerProps } from './AnatomyViewer';
export { AnatomyViewport } from './AnatomyViewport';
export { AnatomyMuscleSelector } from './AnatomyMuscleSelector';
export type { AnatomyMuscleSelectorProps } from './AnatomyMuscleSelector';
export { MuscleLayer } from './MuscleLayer';
export { RecoveryInspector } from './RecoveryInspector';
export { RecoveryLegend } from './RecoveryLegend';
export { SegmentedControl } from './SegmentedControl';
export { useAnatomyTracker } from './useAnatomyTracker';
export { MUSCLE_CANONICAL } from './anatomy.canonicalMap';
export { CANONICAL_TO_GRUPO, canonicalsToGrupos } from './anatomy.groupMap';
export type {
  AnatomyView,
  Gender,
  LegendItem,
  MuscleMap,
  RecoveryLevel,
  RecoveryState,
  SegmentOption,
} from './anatomy.types';
